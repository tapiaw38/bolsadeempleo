    ...
    function ListService(){
      ...
      success: function(response){
        ...
        $.each(response, function(index,element){
          ...
          html += facebook,
          html +='</div>',
          html +='<a href="" data-toggle="modal" data-target="#modal'+element.id+'">',
          html +='<img class="img-fluid mt-3 border rounded" src="'+element.picture_logo+'" alt="'+element.title+'"><span class="badge badge-primary">'+element.category+'</span></a>',
          html +='<p class="mt-1 mb-0 text-primary">'+element.description+'</p>',
          html +=' <form class="form-inline">',
          html +=directions,
          ...
          modal = '<div class="modal fade" id="modal'+element.id+'" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">';
          modal+= '<div class="modal-dialog modal-lg" role="document">';
          modal+= ' <div class="modal-content">';
          modal+= '<div class="modal-header">';
          modal+= ' <h5 class="modal-title" id="'+element.user+'">'+ '@'+element.user+' | '+element.title+'</h5>';
          modal+= '<button type="button" class="close" data-dismiss="modal" aria-label="Close">';
          modal+= '<span aria-hidden="true">&times;</span>';
          modal+= '</button>';
          modal+= '</div>';
          modal+= ' <div class="modal-body">';
          modal+= '<img class="img-fluid border rounded" src="'+element.picture_logo+'" alt="'+element.title+'"> </div>';
          modal+= '</div>';
          modal+= '</div>';
          modal+= '</div>';
          modal+= '';

          $("#modal_json").append(modal);
        })//end each
